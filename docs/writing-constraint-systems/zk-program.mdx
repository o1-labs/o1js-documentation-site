...
for full type information, but two feautures that the `ZkProgram` gives you are access to the methods you define on
...
Private output is more often used as a developer utility, for instance to return some non-provable data that's 
