...
type Transaction<Proven, Signed> = TransactionCommon & {
  ...
  setFeePerAccountUpdate: TransactionPromise<Proven, false>;
...
Defined in: [lib/mina/v1/transaction.ts:91](https://github.com/o1-labs/o1js/blob/cc18a919fe40be152afb1b603c57d74a9b2225b0/src/lib/mina/v1/transaction.ts#L91)
...
### setFeePerAccountUpdate()

```ts
setFeePerAccountUpdate(newFeePerAccountUpdate: number): TransactionPromise<Proven, false>;
```

setFeePerAccountUpdate behaves identically to Transaction.setFee but the fee is given per estimated cost of snarking the transition as given by getSegmentsAndEvents. This is useful because it should reflect what snark workers would charge in times of network contention.

#### Parameters

##### newFeePerAccountUpdate
