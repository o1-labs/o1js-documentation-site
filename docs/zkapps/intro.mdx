...
In order to interact on chain, ZkApps have to deal with several spectial types of data.  There are public keys of users.  There are fees and balances and tokens.  There are slots and epochs, and other smart contracts, etc...  The language that o1js uses to communicate about all these things is called "Accout Updates", and there is a specific article about them [here](/docs/zkapps/account-updates.mdx).
...
ZkApp methods are decorated with `@method`, which tells o1js that this method ought to be part of the constraint system constructed for the ZkApp.  You can add other class methods to a `SmartContract` that aren't a part of the contraint system like `toString()`.  `@method`s must be fully provable like a `ZkProgram`.
