...
1. Known size (fixed number of bits)
2. Bounded (within their type's range)
3. Representable as field elements
4. Constrained (have valid proof rules)
:::

:::note Circuit Structure
Key points about circuit structure:
1. All array sizes must be known when the circuit is compiled
2. Loop bounds must be compile-time constants
3. The entire circuit structure must be determined before proving
:::
...
:::note Understanding Metaprogramming
What is metaprogramming in o1js?
- JavaScript control flow (if/else, loops) runs at compile time
- These statements shape how the circuit will be structured
- Different branches generate different sets of constraints
- The final circuit structure is fixed before proving begins
:::
...
| Test Case | Key Length | Circuit Structure | Total Constraints |
|-----------|------------|-------------------|------------------|
| Case 2    | 4 bytes    | Padding only      | 21,214          |
| Case 1    | 20 bytes   | Padding only      | 21,136          |
| Case 6    | 131 bytes  | SHA-256 + padding | 37,228          |
| Case 7    | 131 bytes  | SHA-256 + padding | 47,859          |
...
```typescript
// 1. Creating fixed-size arrays for key and message processing
const k0Uint32 = Provable.Array(UInt32, 16).empty();    // For 32-bit words
const k0Bytes = Provable.Array(UInt8, BLOCK_SIZE).empty(); // For bytes


// 2. Copying bytes into a Provable array
for (let i = 0; i < BLOCK_SIZE; i++) {
  k0Bytes[i] = k0.bytes[i];  // Copy from regular Bytes to Provable array
}


// 3. Building the inner block (k0 ^ ipad || message)
const innerBlock = Provable.Array(UInt8, BLOCK_SIZE + messageBytes.length).empty();
...
for (let i = 0; i < messageBytes.length; i++) {
  innerBlock[BLOCK_SIZE + i] = messageBytes[i];
}
```
...
```typescript
// 1. Converting bytes to words (little-endian)
for (let i = 0; i < 16; i++) {
...
// 2. XOR operations with padding constants
const k0IpadXor = Provable.Array(UInt32, 16).empty();
for (let i = 0; i < 16; i++) {
  k0IpadXor[i] = k0Uint32[i].xor(IPAD);  // IPAD = 0x36363636
}


// 3. Converting words back to bytes
const k0IpadBytes = Provable.Array(UInt8, BLOCK_SIZE).empty();
for (let i = 0; i < 16; i++) {
...
To use this HMAC implementation in your ZkProgram:
