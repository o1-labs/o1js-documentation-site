...
were executed, with a specific outcome.  Since a single proof can't accept a dynamically sized input, we can use recursion to 
...
does one repetitive task several times.  Although a single constraint system can't have branching logic, you can branch
at a system level by using proofs that verify other proofs. In this example, we handle inputs differently based on user
eligibility determined via public keys.
