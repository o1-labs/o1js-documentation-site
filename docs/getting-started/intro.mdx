...
o1js balances browser support and feature richness.  Most modern browsers, including many mobile browesers
are supported.  We use `WebAssembly` and `SharedArrayBuffer` to improve prover performance on the browser,
which does impact compatibility with older browers.  See below for a full list of supported browsers.
